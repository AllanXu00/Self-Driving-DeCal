...
def conv2d_grayscale(img, ker):
    ...
    for y in range(Ny):
        for x in range(Nx):      
            feature_map[y, x] += np.sum(img[y:y + j, x:x + i] * ker)
    # !!! ==============
...
def conv2d(img, ker):
    ...
    for x in range(Nx):
        for y in range(Ny):
            for c in range(Nc):
                feature_map[y, x] += np.sum(img[y:y + j, x:x + i, c] * ker[:, :, c])
    # !!! ==============
...
def conv2d_many(img, kers):
    ...
    #
    # !!! YOUR CODE HERE
    feature_maps = [conv2d(img, kers[k]) for k in range(len(kers))]
    # !!! ==============
...
def conv2d_padding(img, kers):
    ...
    return conv2d_many(np.pad(img, padding, 'constant', constant_values=0), kers)
...
def conv2d_full(img, kers, stride=1):
    ...
    for ker in kers:
        ...
        for x in range(Nx):
            for y in range(Ny):
                for c in range(kc):
                    feature_map[y, x] += np.sum(img[y*stride:y*stride + j, x*stride:x*stride + i, c] * ker[:, :, c])
        # !!! ==============
